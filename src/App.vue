<template>
  <div id="app" class="container p-5">
    ...
    <div class="card">
      <div class="card-header d-flex flex-row justify-content-between"><span>Todo list</span><span>{{ todosLength }}</span></div>
      <div class="card-body">
        <ul class="list-group">
          <li @click="toggleTodo(index)" v-for="(todo, index) in todos" :key="todo.name" class="list-group-item d-flex flex-row justify-content-between align-items-center list-group-item-action" >
            <span>{{ todo.name }}</span>
            <span>
